...
public:
  ...
  sudoku_puzzle toResult() {
    sudoku_puzzle out;
    for(int i=0; i<9; i++) {
      for(int j=0; j<9; j++) {
        out.grid[i][j] = this->grid[i][j];
        out.solution[i][j] = this->solnGrid[i][j];
      }
    }
    out.difficulty = this->difficultyLevel;
    return out;
  }
};
...
sudoku_puzzle gen_sudoku_puzzle()
{
  // Initialising seed for random number generation
  random_device rd;
  mt19937_64 mt_rand_inst(rd());
  mt_rand = &mt_rand_inst;
